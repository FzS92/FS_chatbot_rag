...
- Users can ask financial questions, and the chatbot responds based on predefined models, Google search, semantic search, and summarization.
...
- `google_search`: Perform a Google search for the user's query or its summarization (if it was long) and save the URLs for specified number of results.
...
- `get_top_n_cosine_similarity_rows`: Get the indices of the top N rows with highest cosine similarity scores.
- `semantic_search`: Perform semantic search by measuring the similarity between encoded of paragraphs or sentences in a corpus and the query.
